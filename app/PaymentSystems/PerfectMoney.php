...
class PerfectMoney {
	...
	public static function render($data = [], $view = 'perfectmoney')
	{
		...
		if(config('perfectmoney.suggested_memo') || isset($data['SUGGESTED_MEMO']))
		{
			...
			if(isset( $data['SUGGESTED_MEMO'] ))
			{
				$view_data['MEMO'] = $data['SUGGESTED_MEMO'];
			}

		}

		// Custom view
		if(view()->exists('paymentsystems.' . $view)){
			return view('paymentsystems.' . $view, $view_data);
		}


		// Default view
		return view('paymentsystems.perfectmoney', $view_data);
	}
	...
	public function isValid($accountId)
	{

		// Get data from the server
		$url = file_get_contents('https://perfectmoney.is/acct/acc_name.asp?AccountID=' . $this->account_id . '&PassPhrase=' . $this->passphrase . '&Account=' . $accountId, false, stream_context_create($this->ssl_fix));
		if($url === false)
		{
		   return ['status' => 'error', 'message' => 'Connection error'];
		}

		if(substr($url, 0, 6) == "ERROR:")
		{
			return ['status' => 'error', 'message' => substr($url, 7)];
		}
